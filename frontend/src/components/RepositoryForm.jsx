...
export default function RepositoryForm({ onAnalysisStarted, toast, findExistingByUrl, onAlreadyScanned, onScanModeChange }) {
  ...
  return (
    <form onSubmit={handleSubmit}>
      <div className="repo-analyze-row" style={{ display: 'flex', gap: 10 }}>
        {/* Input + GitHub icon now share their own positioning context, so
            the icon stays pinned to the input's own height/center whether
            the button sits beside it (desktop) or wraps below it (mobile).
            Previously the icon was absolutely positioned against the whole
            row, so once the row wrapped taller on phones the icon drifted
            away from the input it was meant to sit inside. */}
        <div className="repo-input-wrap" style={{ position: 'relative', flex: 1, minWidth: 0 }}>
